...
{
G4ThreeVector ppre = fStep->GetPreStepPoint()->GetPosition();
G4ThreeVector mpre = fStep->GetPreStepPoint()->GetMomentum();
...
msg << "new step from (" << ppre.x() << "," << ppre.y() << "," << ppre.z() << ")"
    << " in volume " << volname << ":" << copyno
    << " by particle " << fStep->GetTrack()->GetParticleDefinition()->GetParticleName()
    << " with momentum (" << mpre.x() << "," << mpre.y() << "," << mpre.z() << ")"; 
}
...
{
G4ThreeVector ppost = fStep->GetPostStepPoint()->GetPosition();
G4ThreeVector mpost = fStep->GetPostStepPoint()->GetMomentum();
G4TouchableHandle touch = GlueXPathFinder::CreateTouchableHandle();
G4VPhysicalVolume *pvol = (touch)? touch->GetVolume() : 0;
G4String volname = (pvol)? pvol->GetName() : "NULL";
int copyno = (touch)? touch->GetCopyNumber() : 0;
msg << " to (" << ppost.x() << "," << ppost.y() << "," << ppost.z() << ")"
    << " in " << volname << ":" << copyno;
}
