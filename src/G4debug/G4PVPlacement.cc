...
message << G4endl
        << "RTJ - This was a point generated on the surface of volume " << GetLogicalVolume()->GetName()
        << " (" << point[0] << "," << point[1] << "," << point[2] << ")" << G4endl
        << "      In mother coordinates " << motherLog->GetName() << " this point is "
        << " (" << mp[0] << "," << mp[1] << "," << mp[2] << ")" << G4endl
        << "      Now we consider sibbling volume " << daughter->GetName() << " in which this point is "
        << " (" << md[0] << "," << md[1] << "," << md[2] << ")" << G4endl
        << "      and that is considered to be inside the sibbling, triggering this overlap warping."
        << G4endl
        << "      For reference, here are the dimensions of the volumes considered here" << G4endl
        << "      " << motherLog->GetName() << " is "
        << ((G4Box*)motherSolid)->GetXHalfLength() * 2 << " x "
        << ((G4Box*)motherSolid)->GetYHalfLength() * 2 << " x "
        << ((G4Box*)motherSolid)->GetZHalfLength() * 2 << G4endl
        << "      " << daughter->GetName() << " is "
        << ((G4Box*)daughterSolid)->GetXHalfLength() * 2 << " x "
        << ((G4Box*)daughterSolid)->GetYHalfLength() * 2 << " x "
        << ((G4Box*)daughterSolid)->GetZHalfLength() * 2 << G4endl
        << "      " << GetLogicalVolume()->GetName() << " is "
        << ((G4Box*)solid)->GetXHalfLength() * 2 << " x "
        << ((G4Box*)solid)->GetYHalfLength() * 2 << " x "
        << ((G4Box*)solid)->GetZHalfLength() * 2 << G4endl;
