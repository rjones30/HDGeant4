...
msg << "LocateGlobalPointAndSetup called for globalPoint ("
    << globalPoint.x() << "," << globalPoint.y() << "," << globalPoint.z() << ")"
    << ", r=" << globalPoint.perp() << ", sector=" << globalPoint.getPhi()*48/(2*3.1415926)
    << " direction (";
if (pGlobalDirection != 0)
 msg   << pGlobalDirection->x() << "," << pGlobalDirection->y() << "," << pGlobalDirection->z() << ")";
else
 msg << ")";
msg << " relativeSearch " << relativeSearch << " ignoreDirection " << ignoreDirection;
...
details << "testing placed volume " << fHistory.GetTopVolume()->GetName()
        << " for (local) point (" 
...
details << "testing replica volume " << fHistory.GetTopVolume()->GetName()
        << " for (local) point (" 
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << "): ";
if (insideCode == kOutside)
   details << "outside;";
...
details << "knownContained in " << fHistory.GetTopVolume()->GetName() << ":" << fHistory.GetTopVolume()->GetCopyNo()
        << " at local point (" << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << "); ";
  //
...
details << "search contents of volume " << targetPhysical->GetName();
      ...
      case kNormal:
        if ( targetLogical->GetVoxelHeader() )  // use optimised navigation
        {
          noResult = fvoxelNav.LevelLocate(fHistory,
                                           ...
                                           considerDirection,
                                           localPoint);
details << " using fvoxelNav returns " << noResult << " for local point ("
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << ")";
        }
        else                       // do not use optimised navigation
        {
          noResult = fnormalNav.LevelLocate(fHistory,
                                            ...
                                            considerDirection,
                                            localPoint);
details << " using fnormalNav returns " << noResult << " for local point ("
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << ")";
        }
        break;
      case kReplica:
        noResult = freplicaNav.LevelLocate(fHistory,
                                           ...
                                           considerDirection,
                                           localPoint);
details << " using freplicaNav returns " << noResult << " for local point ("
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << ")";
        break;
      case kParameterised:
        if( GetDaughtersRegularStructureId(targetLogical) != 1 )
        {
          noResult = fparamNav.LevelLocate(fHistory,
                                           ...
                                           considerDirection,
                                           localPoint);
details << " using fparamNav returns " << noResult << " for local point ("
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << ")";
        }
        else  // Regular structure
        {
          noResult = fregularNav.LevelLocate(fHistory,
                                             ...
                                             considerDirection,
                                             localPoint);
details << " using fregularNav returns " << noResult << " for local point ("
        << localPoint.x() << "," << localPoint.y() << "," << localPoint.z() << ")";
        }
        break;
    }
...
if (noResult)
 details << " so descending into " << fHistory.GetTopVolume()->GetName() << ":"
         << fHistory.GetTopVolume()->GetCopyNo();
else
 details << " so this is the end of the line!";
...
if (targetPhysical->GetName() == "sd20" || targetPhysical->GetName() == "sd20::1")
GlueXUserEventInformation::Dlog(details.str());

msg << " and found volume " << targetPhysical->GetName() << ":" << targetPhysical->GetCopyNo();
