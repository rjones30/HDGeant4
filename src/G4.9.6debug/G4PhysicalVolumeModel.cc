...
G4cout << "Computing sectionPlane shape for volume " << pSol->GetName() << std::endl;
G4cout << "   transformation operator is as follows:" << std::endl;
G4cout << std::setw(12) << theAT.xx() << std::setw(12) << theAT.xy()
       << std::setw(12) << theAT.xz() << std::setw(20) << theAT.dx() << std::endl;
G4cout << std::setw(12) << theAT.yx() << std::setw(12) << theAT.yy()
       << std::setw(12) << theAT.yz() << std::setw(20) << theAT.dy() << std::endl;
G4cout << std::setw(12) << theAT.zx() << std::setw(12) << theAT.zy()
       << std::setw(12) << theAT.zz() << std::setw(20) << theAT.dz() << std::endl;
G4cout << "   with a phi rotation angle of " << atan2(theAT.xy(),theAT.xx())*180/3.1416 << std::endl;
#endif
...
#ifdef DEBUG_SECTIONPLANE_ZAVE
if (tmpResultant && resultant.GetNoFacets() > 0) {
   float zave=0;
   int nvert = resultant.GetNoVertices();
   for (int v=1; v <= nvert; ++v) {
      G4Point3D vtx = resultant.GetVertex(v);
      zave += vtx[2];
   }
}
zave /= resultant.GetNoVertices();
G4cout << "primitive polygon with " << nvert << " vertices"
       << ", average z = " << zave << std::endl;
}
#endif
